...
class ICM20948(object):
    def __init__(self, address=I2C_ADD_ICM20948):
        ...
        self._write_byte(REG_ADD_GYRO_CONFIG_1, REG_VAL_BIT_GYRO_DLPCFG_6 |
                         REG_VAL_BIT_GYRO_FS_1000DPS | REG_VAL_BIT_GYRO_DLPF)
        self._write_byte(REG_ADD_ACCEL_SMPLRT_DIV_2,  0x07)
        self._write_byte(REG_ADD_ACCEL_CONFIG, REG_VAL_BIT_ACCEL_DLPCFG_6 |
        ...
        self.icm20948WriteSecondary(
            I2C_ADD_ICM20948_AK09916 | I2C_ADD_ICM20948_AK09916_WRITE, REG_ADD_MAG_CNTL2, REG_VAL_MAG_MODE_20HZ)
    ...
    def icm20948MagRead(self):
        ...
        while(counter > 0):
            ...
            self.icm20948ReadSecondary(
                I2C_ADD_ICM20948_AK09916 | I2C_ADD_ICM20948_AK09916_READ, REG_ADD_MAG_ST2, 1)
            if ((pu8data[0] & 0x01) != 0):
                break
            counter -= 1
        if counter != 0:
            for i in range(0, 8):
                self.icm20948ReadSecondary(
                    I2C_ADD_ICM20948_AK09916 | I2C_ADD_ICM20948_AK09916_READ, REG_ADD_MAG_DATA, MAG_DATA_LEN)
    ...
    def icm20948ReadSecondary(self, u8I2CAddr, u8RegAddr, u8Len):
        u8Temp = 0
        self._write_byte(REG_ADD_REG_BANK_SEL,
                         REG_VAL_REG_BANK_3)  # swtich bank3
        ...
        self._write_byte(REG_ADD_REG_BANK_SEL,
                         REG_VAL_REG_BANK_0)  # swtich bank0
    ...
    def imuAHRSupdate(self, gx, gy, gz, ax, ay, az, mx, my, mz):
        ...
        hx = 2 * mx * (0.5 - q2q2 - q3q3) + 2 * my * \
            (q1q2 - q0q3) + 2 * mz * (q1q3 + q0q2)
        hy = 2 * mx * (q1q2 + q0q3) + 2 * my * \
            (0.5 - q1q1 - q3q3) + 2 * mz * (q2q3 - q0q1)
    ...
    def icm20948MagCheck(self):
        self.icm20948ReadSecondary(
            I2C_ADD_ICM20948_AK09916 | I2C_ADD_ICM20948_AK09916_READ, REG_ADD_MAG_WIA1, 2)
...
#         roll = math.atan2(2 * q2 * q3 + 2 * q0 * q1, -2 *
#                           q1 * q1 - 2 * q2 * q2 + 1) * 57.3
#         yaw = math.atan2(-2 * q1 * q2 - 2 * q0 * q3, 2 *
#                          q2 * q2 + 2 * q3 * q3 - 1) * 57.3
#         print("\r\n /-------------------------------------------------------------/ \r\n")
...
def getGyro():
    ...
    yaw = math.atan2(-2 * q1 * q2 - 2 * q0 * q3, 2 *
                        q2 * q2 + 2 * q3 * q3 - 1) * 57.3
    results = []
    results.append([(Mag[0]), Mag[1], Mag[2]])
    results.append([Accel[0], Accel[1], Accel[2]])
    results.append([Gyro[0], Gyro[1], Gyro[2]])
    results.append([yaw, pitch, roll])
    # print(results)
    return results
