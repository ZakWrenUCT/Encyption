...
def benchmark_processing():
    ...
    for i in range(10):
        ...
        for i in range(row_count):
            data = data_source.next()
            data.insert(0, current_time + timedelta(seconds=sampling_rate*i))
            batch_rows.append(data)
        ...
        encSize = os.path.getsize(enc_filename)
        print("Compression ratio:", compRatio)

        benchmarks.append((data_duration, t_compression, oriSize, compSize, compRatio, encSize,
        ...
        print("data_duration,t_compression,oriSize,compSize,compRatio,encSize,t_encryption,t_decryption,t_decompression")
        for b in benchmarks:
